...
function searchPageInit() {
  const allCardItems = Array.from(
    document.getElementsByClassName("card-grid-item")
  );
  const actualCardItems = allCardItems.filter(
    (elem) => !elem.getAttribute("aria-hidden")
  );
  if (actualCardItems.length == 0 && retries < 3) {
    console.debug(
      `No cards detected, sleeping and trying ${3 - retries} more times`
    );
    retries++;
    sleep(500).then(init);
...
function initCardElement(element) {
  ...
  const transformButtonElement = document.querySelectorAll(
    "[data-component='card-backface-button']"
  )[0];
...
function makeAddButton(element, isSelected = false) {
  ...
  button.onclick = () =>
    handleAddButtonClick(button, getCardName(element), getCardLink(element));
...
function makeButtonRow() {
  ...
  container.appendChild(
    makeImageButton("duplicate", "Copy selected cards", (btn) =>
      copyClipboard(btn)
    )
  );
  container.appendChild(
    makeImageButton("trash", "Clear card clipboard", clearClipboard)
  );
  container.appendChild(
    makeImageButton("clip", "View card clipboard", toggleClipboardList)
  );
...
function loadClipboardFromStorage() {
  ...
  if (cardList.length > 0) {
    try {
      ...
      console.debug(
        `Loaded ${cardsArr.length} cards from local storage: ${cardsArr.map(
          (card) => card.cardName
        )}`
      );
      return cardsArr;
      ...
      console.warn(
        "Errored loading cards from local storage. Clearing stored clipboard"
      );
      localStorage.removeItem("cardClipboard");
    }
  }
...
function getSingleCardName() {
  ...
  const cardNameElements = Array.from(
    document.getElementsByClassName("card-text-card-name")
  );
...
function findExistingButtons(element) {
  ...
  return existingButtons.filter((elem) =>
    ["unselected", "selected"].includes(elem.className)
  );
}
